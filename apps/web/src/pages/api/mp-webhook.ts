...
export const POST: APIRoute = async ({ request }) => {
  ...
  if (payment.status === 'approved') {
    ...
    const { error: insertError } = await supabase.from('payments').insert({
      gift_id:            payment.metadata?.gift_id ?? null,
    ...
    if (insertError) {
      console.error('[mp-webhook] Erro ao salvar pagamento:', insertError)
    }

    if (payment.metadata?.gift_id) {
      const { error: updateError } = await supabase
        .from('gifts')
        .update({ is_available: false })
        .eq('id', payment.metadata.gift_id)

      if (updateError) {
        console.error('[mp-webhook] Erro ao atualizar gift:', updateError)
      }
    }
  }
